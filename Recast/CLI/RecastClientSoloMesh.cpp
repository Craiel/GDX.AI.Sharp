...
RecastWrapper::RecastClientSoloMesh::RecastClientSoloMesh()
{	
}
...
void RecastWrapper::RecastClientSoloMesh::buildStep1InitConfig()
{
	...
	// Here the bounds of the input mesh are used
	rcVcopy(m_cfg.bmin, bmin);
	rcVcopy(m_cfg.bmax, bmax);
	rcCalcGridSize(m_cfg.bmin, m_cfg.bmax, m_cfg.cs, &m_cfg.width, &m_cfg.height);
}

bool RecastWrapper::RecastClientSoloMesh::buildStep2Rasterize()
{
	const float* verts = m_geom->getMesh()->getVerts();
	const int nverts = m_geom->getMesh()->getVertCount();
	const int* tris = m_geom->getMesh()->getTris();
	...
	rcMarkWalkableTriangles(m_ctx, m_cfg.walkableSlopeAngle, verts, nverts, tris, ntris, triareas);
	if (!rcRasterizeTriangles(m_ctx, verts, nverts, tris, triareas, ntris, *m_solid, m_cfg.walkableClimb))
	{
...
void RecastWrapper::RecastClientSoloMesh::buildStep3FilterWalkable()
{
...
bool RecastWrapper::RecastClientSoloMesh::buildStep4PartitionWalkableSurface()
{
...
bool RecastWrapper::RecastClientSoloMesh::buildStep5TraceAndSimplify()
{
	// Create contours.
...
bool RecastWrapper::RecastClientSoloMesh::buildStep6BuildPolygons()
{
...
bool RecastWrapper::RecastClientSoloMesh::buildStep7CreateDetailMesh()
{
...
bool RecastWrapper::RecastClientSoloMesh::buildStep8CreateDetourData()
{
...
bool RecastWrapper::RecastClientSoloMesh::doBuild()
{
	// Step 2. Rasterize input polygon soup
	if (!buildStep2Rasterize())
	{
		return false;
	}

	// Step 3. Filter walkables surfaces.
	buildStep3FilterWalkable();

	// Step 4. Partition walkable surface to simple regions.
	if (!buildStep4PartitionWalkableSurface())
	{
		return false;
	}

	// Step 5. Trace and simplify region contours.
	if (!buildStep5TraceAndSimplify())
	{
		return false;
	}

	// Step 6. Build polygons mesh from contours.
	if (!buildStep6BuildPolygons())
	{
		return false;
	}

	// Step 7. Create detail mesh which allows to access approximate height on each polygon.
	if (!buildStep7CreateDetailMesh())
	{
		return false;
	}
